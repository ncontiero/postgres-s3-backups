...
export async function deleteOldBackups() {
  if (env.BACKUP_RETENTION_DAYS === undefined) {
    logger.info("No backup retention policy set, skipping cleanup.");
    return;
  }

  logger.info(
    `Checking for backups older than ${env.BACKUP_RETENTION_DAYS} days...`,
  );
  ...
  try {
    ...
    const retentionDaysInMs = env.BACKUP_RETENTION_DAYS * 24 * 60 * 60 * 1000;
    const cutoffDate = new Date(now.getTime() - retentionDaysInMs);
